...
if [ "$pull_status" -ne 0 ]; then
  if printf '%s' "$pull_output" | grep -Eqi 'timed out|timeout|temporary failure|could not resolve host|connection reset|connection timed out|TLS|HTTP 5[0-9][0-9]|502|503|504|remote end hung up|failed to connect|network is unreachable'; then
    sleep 5
    pull_status=0
    pull_output=$(git -C /home/aheinen/.openclaw/workspace/k8s-2025 pull --ff-only 2>&1) || pull_status=$?
  fi
...
if ! git diff --cached --quiet; then
  ...
  commit_msg="chore: refresh backup $(date +%F)"
  git commit -m "$commit_msg"
  commit_hash=$(git rev-parse --short HEAD)
  commit_created=yes
  push_output=$(git push 2>&1) || push_status=$?
  if [ "$push_status" -ne 0 ]; then
    if printf '%s' "$push_output" | grep -Eqi 'timed out|timeout|temporary failure|could not resolve host|connection reset|connection timed out|TLS|HTTP 5[0-9][0-9]|502|503|504|remote end hung up|failed to connect|network is unreachable'; then
      sleep 5
      push_status=0
      push_output=$(git push 2>&1) || push_status=$?
    fi
