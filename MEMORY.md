...
- **k8s-2025 reorg planning (Apr 1-3):** Draft assessment/proposal/transition-plan/naming-convention docs live under `plans/`; repo is aligned with `origin/main`; validation now confirms the repo has already been substantially reorganized, so the work shifted from target-design to validating the executed structure and identifying residual cleanup. Active AWX naming cleanup appears to have landed, leaving mostly filename-level and placement-level judgment calls (`docs/` root docs, conservative playbook filenames, and pragmatic top-level exceptions like `cloud-init-templates/`, `scripts/`, and `kubernetes/`). The proposal is now presentation-ready as an explanation of the current structure and rationale, with further cleanup intended to be selective rather than a compulsive rename spree.
...
## Recent Updates
- **Apr 3:** `k8s-2025` reorg planning was re-baselined against the live repo: high-level structure is already in place, most transition stages were effectively executed on Apr 2, and the planning docs now serve mainly as validation plus residual-cleanup guidance. Proposal is presentation-ready; naming cleanup should stay selective. A fresh inventory delta also reconfirmed `k8s6.lan` SSH failure and `172.16.1.16` as an unresolved inventory stray. Quant short-term validation was also tightened into a genuinely rolling 5-year train / next-year test; result: the short-term edge looks worse, not better, and remains research material rather than something fit to brag about. Ops follow-up also confirmed the cron-delivery issue persists (`announce` + missing `delivery.to`), duplicate-run oddities exist on a few jobs after schedule churn, and the handled `python` / shell-formatting errors still deserve cleanup.
