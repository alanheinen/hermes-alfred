...
## Recent Updates
- **Apr 5:** `k8s-2025` documentation remediation was executed and pushed as seven commits on `main`: broken links went from 19 to 0, `ansible/README.md` became an operator guide, directory indexes and a service map were added, and recovery/archive docs were labeled more honestly. Remaining follow-up is now much smaller: fix the stale Terraform target in `scripts/deploy-homeassistant-pxe.sh`, scrub old Terraform target names from a few docs, and run live Ansible/Terraform validation somewhere that actually has those binaries.
...
- **Mar 26-28:** Infrastructure monitoring week — deployed ntfy-alert-listener daemon (systemd, auto-restart, flap filtering), investigated Frigate OOM pattern (4th+ incident), auto-restart logic with 10-min cooldown, granted SSH access (OPNsense + vm01, read-only + Frigate restart exception), filed multiple RCAs. vm01 drive replacement started Mar 27 (may reduce the I/O pressure behind the issue). Token optimization: 59% context reduction. Quant strategy paper publication-ready (12.39% CAGR, 0.73 Sharpe, +187 bps over SPY).
