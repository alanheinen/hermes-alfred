...
if [ "$pull_status" -ne 0 ]; then
  if printf '%s' "$pull_output" | grep -Eqi 'Could not resolve host|Connection timed out|Operation timed out|TLS|SSL|Failed to connect|Connection reset|remote end hung up|502|503|504|temporar|timeout'; then
    sleep 5
    pull_status=0
    pull_output=$(git -C /home/aheinen/.openclaw/workspace/k8s-2025 pull --ff-only 2>&1) || pull_status=$?
  fi
...
if ! git diff --cached --quiet; then
  ...
  if [ "$push_status" -ne 0 ]; then
    if printf '%s' "$push_output" | grep -Eqi 'Could not resolve host|Connection timed out|Operation timed out|TLS|SSL|Failed to connect|Connection reset|remote end hung up|502|503|504|temporar|timeout'; then
      sleep 5
      push_status=0
      push_output=$(git push 2>&1) || push_status=$?
    fi
